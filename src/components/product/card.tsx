...
export const ProductCard = ({ product, initialWishlist }: Props) => {
  const [isWishlist, setIsWishlist] = useState(initialWishlist ?? false);
  const { user } = useAuth();
  ...
  return (
    ...
    >
      <div>
        <img
          className="w-35 h-35 object-cover mb-4 rounded"
